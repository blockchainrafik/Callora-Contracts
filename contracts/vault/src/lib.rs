...
pub enum VaultError {
    ...
    /// Withdrawal recipient cannot be the vault contract itself (code 30).
    WithdrawToVaultAddress = 30,
    /// Withdrawal recipient cannot be the USDC token contract (code 31).
    WithdrawToTokenAddress = 31,
    /// Settlement address cannot be the vault contract itself (code 32).
    SettlementCannotBeVault = 32,
    /// Settlement address cannot be the USDC token contract (code 33).
    SettlementCannotBeUsdcToken = 33,
    /// Settlement address cannot equal the configured revenue pool address (code 34).
...
impl CalloraVault {
    ...
    pub fn withdraw_to(env: Env, to: Address, amount: i128) -> Result<i128, VaultError> {
        ...
        let ua: Address = env
            ...
            .ok_or(VaultError::NotInitialized)?;
        if to == ua {
            return Err(VaultError::WithdrawToTokenAddress);
        }
    ...
    pub fn set_settlement(
        ...
        let usdc: Address = env
            ...
            .get(&StorageKey::UsdcToken)
            .ok_or(VaultError::NotInitialized)?;
        if settlement_address == usdc {
            return Err(VaultError::SettlementCannotBeUsdcToken);
        }
        if let Some(pool) = env.storage().instance().get(&StorageKey::RevenuePool) {
            if settlement_address == pool {
